...
static inline void play_end_turn_sound()
{
    play_sqr_wave_sound(G_NOTE, 0, 15);
}

static inline void play_win_animation_noise()
{
...
static void draw_win_animation(int8 dx, int8 dy, void* row, void* column)
{
    int x, y;
...
static uint8 check_for_winner()
{
    ...
    if(sprites[0]!=PLACEHOLDER_TID)
    {
        ...
        {
            winner =  sprites[0];
            draw_win_animation(5, 0, 1, NULL);
        }
        if(!(sprites[0]^sprites[4]) & !(sprites[4]^sprites[8]))
        {
            winner =  sprites[0];
            draw_win_animation(5, 4, 1, NULL);
        }
        if(!(sprites[0]^sprites[3]) & !(sprites[3]^sprites[6]))
        {
            winner =  sprites[0];
            draw_win_animation(0, 5, NULL, 1);
        }
    }
