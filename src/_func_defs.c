...
void set_video_mode3_sound_channel1()
{
...
static inline uint8 valid_channel(void* sound_channel)
{
    uint8 channel = (uint8)sound_channel;
    channel &=0xF;
    return channel!=NULL && (!(channel^SOUND_DMG_SQUARE_1) | !(channel^SOUND_DMG_SQUARE_2) | !(channel^SOUND_DMG_WAVE) | !(channel^SOUND_DMG_NOISE));
}

void set_regs(uint16 video_mode, uint16 bg_mode, uint8 sound_activate, void* sound_channel)
{
    REG_DISPLAY = bg_mode|video_mode;
    if(sound_activate && sound_channel!=NULL)
    {
        REG_SOUND_STAT = MASTER_SOUND_ENABLE;
